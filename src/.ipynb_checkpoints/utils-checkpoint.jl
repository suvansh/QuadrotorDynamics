...
function read_neuroBEM(csv_path)
    ...
    xs = [data[:, 18:20] data[:, 8:11] data[:, 15:17] data[:, 5:7]]
    ẋs = [data[:, 15:17] q̇s data[:, 12:14] data[:, 2:4]]
    us = data[:, 21:24]
    return xs, ẋs, us, dt
end
...
function chunk_traj(xs, ẋs, us, xchunks_out, ẋchunks_out, uchunks_out, chunk_length)
    n = size(xs, 1)
    num_chunks = n ÷ chunk_length
    for i=1:num_chunks
        push!(xchunks_out, xs[1+(i-1)*chunk_length:i*chunk_length, :])
        push!(ẋchunks_out, ẋs[1+(i-1)*chunk_length:i*chunk_length, :])
        push!(uchunks_out, us[1+(i-1)*chunk_length:i*chunk_length, :])
    end
end
...
function get_force_torque(qr, xs, us, ẋs)
    ...
    forces = []
    torques = []
    for (x, u, ẋ) in zip(eachrow(xs), eachrow(us), eachrow(ẋs))
